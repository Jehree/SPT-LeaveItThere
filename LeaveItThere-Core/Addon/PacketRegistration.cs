...
/// <summary>
/// Derivatives are singletons. Get them with PacketRegistration.Get<T>(). Do not instantiate them.
/// <para>To use, create your own class for your packet and inherit the base: MyCustomPacket : PacketRegistration.</para>
/// <para>Register it in your plugin's Awake() function with PacketRegistration.Get<MyCustomPacket>().Register();</para>
/// <para>Send it with  PacketRegistration.Get<MyCustomPacket>().SendData()</para>
...
public abstract class PacketRegistration
{
    ...
    private static Dictionary<Type, PacketRegistration> _instances = [];

    protected PacketRegistration()
    {
        var type = GetType();

        if (_instances.ContainsKey(type))
        {
            throw new InvalidOperationException($"{type.Name} is a singleton and an instance already exists! Do not instantiate PacketRegistration derivatives. Get them with PacketRegistration.Get<YourPacketClassName>().");
        }
    ...
    /// <returns></returns>
    public static T Get<T>() where T : PacketRegistration, new()
    {
        var type = typeof(T);
    ...
    {
        FikaBridge.RegisterGenericPacket(this);
    }

    public void Unregister()
    {
        FikaBridge.UnregisterGenericPacket(PacketGUID);
    }
    ...
    internal void SendPacket(Packet packet)
    {
        ...
        OnPacketSent(packet);
        FikaBridge.SendGenericPacket(packet);
    }
    ...
    {
        if (!LeaveItThereHelper.FikaInstalled) return;
